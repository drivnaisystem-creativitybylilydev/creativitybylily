...
import { SHIPPING_CARRIERS, normalizeCarrier, getCarrierTrackingUrl } from '@/lib/carriers';
...
export default function OrderTrackingFulfillment({
  ...
  const [carrier, setCarrier] = useState(normalizeCarrier(shipmentCarrier));
  ...
  useEffect(() => {
    setCarrier(normalizeCarrier(shipmentCarrier));
  }, [shipmentCarrier]);
  ...
  async function patchStatus(body: { status: OrderStatus; tracking_number?: string | null; carrier?: string }) {
  ...
  return (
    <div className="border-t border-gray-200 pt-6 mt-6">
      ...
      <p className="text-xs text-gray-600 mb-3 leading-relaxed">
        Enter the tracking number from Rollo (or whichever carrier you shipped with) below. The customer is only
        emailed when you click <strong>Mark as shipped &amp; email customer</strong> (saving the number alone does
        not send mail).
      </p>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <div className="sm:w-36">
          <label htmlFor={`carrier-${orderId}`} className="mb-1 block text-sm font-medium text-gray-800">
            Carrier
          </label>
          <select
            id={`carrier-${orderId}`}
            value={carrier}
            onChange={(e) => setCarrier(normalizeCarrier(e.target.value))}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-[color:var(--logo-pink)] focus:outline-none focus:ring-2 focus:ring-[color:var(--logo-pink)]/30"
          >
      ...
      {trimmed && (
        <a
          href={getCarrierTrackingUrl(carrier, trimmed)}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-3 inline-block text-sm text-blue-600 hover:underline"
        >
          Open {SHIPPING_CARRIERS.find((c) => c.value === carrier)?.label ?? 'carrier'} tracking →
        </a>
      )}

      {(shipmentCarrier || shipmentTrackingStatus) && (
        <p className="mt-2 text-xs text-gray-500">
          {shipmentCarrier && <span>Carrier on file: {shipmentCarrier.toUpperCase()}. </span>}
          {shipmentTrackingStatus && <span>Tracking status: {shipmentTrackingStatus}</span>}
        </p>
      )}
    </div>
  );
