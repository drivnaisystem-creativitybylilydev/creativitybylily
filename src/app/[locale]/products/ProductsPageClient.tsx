...
import type { RatingStats } from '@/lib/reviews';
...
function ProductsPageInner({ initialProducts, initialRatings, initialBestsellerIds }: ProductsPageClientProps) {
  ...
  const bestsellerIds = useMemo(() => new Set(initialBestsellerIds), [initialBestsellerIds]);
  ...
  // Skip the very first run: the server component already fetched products matching
  // the initial URL, so the first paint has real content instead of a loading spinner.
